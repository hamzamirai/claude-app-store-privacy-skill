...
Follow these phases in order. Do NOT skip phases — including Phase 0.

> **CRITICAL — Pre-approved plans**: Even when this skill is invoked as part of a pre-approved plan (e.g. "implement the following plan"), Phase 0 is **NON-NEGOTIABLE**. You MUST ask the format question before writing any output files. Do not assume a format. Do not default silently. Stop and ask.

---

### Phase 0: Report Format (MANDATORY — Ask First, Before Any Scanning)
...
If ANY AI SDK or endpoint was detected in Phases 2 or 3, generate a complete **AI Data Practices** section and a standalone `Docs/AppPrivacyReport/AI-PRIVACY-DISCLOSURE.md` file.
...
Use the **`canvas-design` skill** to generate a beautifully designed PDF version at `Docs/AppPrivacyReport/AI-PRIVACY-DISCLOSURE.pdf`, styled with the detected app colors, app name, and icon. Then also generate the plain `Docs/AppPrivacyReport/AI-PRIVACY-DISCLOSURE.md` as a developer-readable copy.
...
If `canvas-design` skill is not available, generate `Docs/AppPrivacyReport/AI-PRIVACY-DISCLOSURE.md` only (plain markdown), with a comment block at the top noting the detected app theme:

```markdown
<!-- App Theme: [App Name] | Primary: #[hex] | Style: [Light/Dark/Vibrant] -->
```

Create `Docs/AppPrivacyReport/AI-PRIVACY-DISCLOSURE.md` with the following structure:
...
- Messaging/chat features detected (`MessageKit`, WebSocket, `Stream`, `SendBird`, Firebase Realtime Database for chat)
- `AVCaptureSession` with uploads

Then **verify moderation and abuse reporting** by grepping for:
- `reportContent`, `reportUser`, `flagContent`, `reportAbuse`, `blockUser`, `muteUser`
- Strings: `"Report"`, `"Block"`, `"Flag"` in localization files
- Content moderation services: `import Perspective`, `import OpenAI` used for content filtering
...
- Strings: `"Manage Subscription"`, `"manageSubscription"` in localization

| Result | Severity | Finding |
|--------|----------|---------|
| Subscription detected + no management link | **WARN** | Apps with auto-renewable subscriptions must provide a way to manage/cancel (§3.1.2) |
| Subscription detected + management link found | ✅ PASS | Subscription management implemented |
| No subscription detected | ✅ N/A | Not required |
...
Also grep all `.swift` files for:
- `"privacyPolicy"`, `"privacy_policy"`, `"privacy-policy"` — any URL reference to a privacy policy

| Result | Severity | Finding |
|--------|----------|---------|
| `NSPrivacyPolicyURL` missing from Info.plist | **WARN** | Apple requires a privacy policy URL — add `NSPrivacyPolicyURL` to Info.plist and set it in App Store Connect |
| `NSPrivacyPolicyURL` present | ✅ PASS | Privacy policy URL configured |
...
Also grep `.swift` files for UI strings matching these patterns.
...
| UGC safety (moderation/reporting) | §1.2 | ... | [e.g. "Chat detected — no abuse reporting found"] |
| Kids category API restrictions | §1.3 | ... | [e.g. "N/A — not a kids app" or "HealthKit banned in kids category"] |
...
### Phase 9.2: Pre-Submission Checklist

After completing all compliance checks, generate a **Pre-Submission Checklist** section to include in the report. This is derived from Apple's "Avoiding Common Issues" guidance at `developer.apple.com/distribute/app-review/` and covers submission-process concerns that cannot be code-detected.

Combine automated check results with manual checklist items:

#### Automated Results (from Phase 9 checks)

Present each automated check result in checklist format:
- ✅ if the check passed
...
| Sign in with Apple parity | ✅/⚠️/🚨 | [from 8f] |
| Subscription management link | ✅/⚠️ | [from 8i] |
| UGC moderation/abuse reporting | ✅/⚠️ | [from 8g] |
...
- [ ] App description in App Store Connect accurately reflects all current features
- [ ] Screenshots and preview videos reflect the current app UI (correct iOS/macOS version, no outdated UI)
- [ ] All placeholder text, test accounts, or debug-only UI removed from the production build
- [ ] Contact/support information is accessible within the app or on the developer website
...
### Phase 9.5: Confirm Output Format

#### Legacy Report Migration (Run First)

Before writing any output files, check whether the project has reports from a previous run saved at the old flat `Docs/` location. If found, move them into `Docs/AppPrivacyReport/`:

```bash
mkdir -p "Docs/AppPrivacyReport"
for f in \
  "Docs/APP-PRIVACY-AND-AGE-RATING-REPORT.docx" \
  "Docs/APP-PRIVACY-AND-AGE-RATING-REPORT.pdf" \
  "Docs/APP-PRIVACY-REPORT.md" \
  "Docs/AI-PRIVACY-DISCLOSURE.md" \
  "Docs/AI-PRIVACY-DISCLOSURE.pdf"; do
  [ -f "$f" ] && mv "$f" "Docs/AppPrivacyReport/"
done
```

If any files were moved, inform the user:
> "Found existing reports in `Docs/` — moved them to `Docs/AppPrivacyReport/` to match the new layout."
...
Generate a `.docx` file named `APP-PRIVACY-AND-AGE-RATING-REPORT.docx` saved to the `Docs/AppPrivacyReport/` folder in the project root (create the folder if it doesn't exist). This keeps a versioned copy alongside the project for future reference.
...
9. **App Store Compliance Findings**
   ...
   - New checks: UGC safety (§1.2), Kids category (§1.3), Subscription management (§3.1.2), Privacy policy URL, Placeholder content (§2.1)
...
13. **Exemptions Applied**
    - List of exempted data types with reasoning

14. **Confidence Notes & Limitations**
...
- **CRITICAL — table helper pattern**: Always pass `colWidths` as a separate array to any `makeTable` / `tableWithHeader` helper. **NEVER** try to read column widths back out of cell objects (e.g. `c._cellProperties._width.size`) — internal docx-js properties are not stable across versions and will throw `TypeError: Cannot read properties of undefined`. The correct pattern is:
  ```js
  // ✅ CORRECT — colWidths passed explicitly
  function makeTable(colWidths, headerLabels, dataRows) {
    return new Table({
      width: { size: CONTENT_W, type: WidthType.DXA },
      columnWidths: colWidths,                        // use the array directly
      rows: [
        new TableRow({ tableHeader: true, children: headerLabels.map((label, i) => makeHeaderCell(label, colWidths[i])) }),
        ...dataRows,
      ],
    });
  }

  // ❌ WRONG — crashes on current docx-js versions
  columnWidths: headerCells.map(c => c._cellProperties._width.size)
  ```
...
- Ensure `Docs/AppPrivacyReport/` directory exists before writing: `fs.mkdirSync('Docs/AppPrivacyReport', { recursive: true })`
- Write output to `Docs/AppPrivacyReport/APP-PRIVACY-AND-AGE-RATING-REPORT.docx`
...
python3 ~/.claude/skills/docx/scripts/office/validate.py \
  Docs/AppPrivacyReport/APP-PRIVACY-AND-AGE-RATING-REPORT.docx
```
...
### Phase 10b: PDF Report Generation

Only run this phase if the format choice from Phase 0 was **C** or **D**.

Uses Python + **reportlab** (Platypus). Pattern mirrors the `market-report-pdf` skill.

**Step 1 — Install reportlab**

```bash
pip3 install reportlab 2>&1 | tail -3
```

If this fails, fall back to `.md` only and inform the user:
> "Could not install `reportlab`. Generating Markdown report only. Run `pip3 install reportlab` and re-run the skill to get the PDF."

**Step 2 — Write the Python generation script**

Use the Write tool to create `/tmp/gen_privacy_report_pdf.py`. The script must:
...
- **Color palette** (define as constants):
  - `COLOR_HEADER = HexColor("#1A3A5C")`
  - `COLOR_ACCENT = HexColor("#2E86C1")`
  - `COLOR_TABLE_HEADER_BG = HexColor("#D6EAF8")`
  - `COLOR_CRITICAL_BG = HexColor("#FADBD8")`
  - `COLOR_WARNING_BG = HexColor("#FDEBD0")`
  - `COLOR_PASS_BG = HexColor("#D5F5E3")`
  - `COLOR_ROW_ALT = HexColor("#EBF5FB")`

- **Custom paragraph styles** (extend `getSampleStyleSheet()`):
  - `ReportTitle` — 24pt, bold, `COLOR_HEADER`, centered, spaceAfter=6
  - `ReportSubtitle` — 12pt, `COLOR_ACCENT`, centered, spaceAfter=4
  - `SectionHeading` — 14pt, bold, `COLOR_HEADER`, spaceBefore=14, spaceAfter=6
  - `SubHeading` — 11pt, bold, `COLOR_ACCENT`, spaceBefore=8, spaceAfter=4
  - `BodyText` — 10pt, leading 14, spaceAfter=4
  - `CodeBlock` — 9pt, `Courier`, leading 12, leftIndent=12, background `#F4F6F7`

- **Header/footer** via `onFirstPage` / `onLaterPages` canvas callbacks:
  - Header: app name (left) + "App Store Privacy Report" (right), 10pt, `COLOR_ACCENT`, with a 0.5pt rule below
  - Footer: "Confidential — Generated {date}" (left) + "Page N" (right), 8pt gray

- **Document structure** (same sections as .docx, populated with all data gathered in earlier phases):

  1. **Title Page** — App name (ReportTitle), "App Store Privacy & Age Rating Report" (ReportSubtitle), date, detected platform(s), then `PageBreak()`
  2. **Executive Summary** — paragraph summary of findings, SDK count, risk level
  3. **Detected SDKs & Frameworks** — `Table` with columns: SDK/Framework | Category | Privacy Impact | Confidence. Header row: `COLOR_ACCENT` bg, white text. Alternating row colors.
  4. **Privacy Declaration by Data Category** — one sub-section per category (e.g. Contact Info, Health, Location…), each with a `Table`: Data Type | Collected | Linked to User | Used for Tracking | Purpose
  5. **App Store Connect Checklist** — `Table`: Requirement | Status | Notes. Status cells color-coded: ✅ → `COLOR_PASS_BG`, ⚠️ → `COLOR_WARNING_BG`, 🚨 → `COLOR_CRITICAL_BG`
  6. **Age Rating Questionnaire** — tables per descriptor group with Yes/No answers and rationale
...
- **Table helper** — define a `make_table(headers, rows, col_widths, status_col=None)` function that:
  ...
  - Applies `ROWBACKGROUNDS` alternating white / `COLOR_ROW_ALT`
  - If `status_col` index given, applies per-cell background based on cell content (🚨 → `COLOR_CRITICAL_BG`, ⚠️ → `COLOR_WARNING_BG`, ✅ → `COLOR_PASS_BG`)
  - Applies `GRID` 0.5pt `HexColor("#BDC3C7")`
  - Wraps all cells in `Paragraph` with `BodyText` style for wrapping

- `os.makedirs('Docs/AppPrivacyReport', exist_ok=True)` before building
- Output path: `Docs/AppPrivacyReport/APP-PRIVACY-AND-AGE-RATING-REPORT.pdf`
- Wrap entire build in `try/except`; on exception: `traceback.print_exc(); sys.exit(1)`
- **Never use Unicode subscript/superscript characters** — use `<sub>` / `<super>` XML tags inside Paragraph strings

**Step 3 — Run the script**

```bash
python3 /tmp/gen_privacy_report_pdf.py
```
...
r = PdfReader('Docs/AppPrivacyReport/APP-PRIVACY-AND-AGE-RATING-REPORT.pdf')
print(f'PDF OK — {len(r.pages)} pages')
" 2>/dev/null || echo "PDF written (pypdf not available for page-count validation)"
```
...
| File | Description |
|------|-------------|
| `Docs/AppPrivacyReport/APP-PRIVACY-AND-AGE-RATING-REPORT.docx` | Professional Word document — privacy, age rating, AI data practices, compliance findings |
| `Docs/AppPrivacyReport/APP-PRIVACY-AND-AGE-RATING-REPORT.pdf` | Professional PDF report — same structure as .docx, generated with Python reportlab |
| `Docs/AppPrivacyReport/APP-PRIVACY-REPORT.md` | Markdown report — no dependencies required |
| `Docs/AppPrivacyReport/AI-PRIVACY-DISCLOSURE.md` | AI-specific disclosure sheet — only generated if AI SDKs detected |
| `<target>/PrivacyInfo.xcprivacy` | Apple privacy manifest — one per platform target |

If multiple platform targets exist, a separate `PrivacyInfo.xcprivacy` is generated for each with only the data types and APIs relevant to that platform. The `Docs/AppPrivacyReport/` folder keeps a versioned history of reports across submissions. The App Store Connect checklist, Age Rating, AI Data Practices, and Compliance Findings are all in the .docx/.pdf reports. `AI-PRIVACY-DISCLOSURE.md` is a standalone shareable sheet developers can paste into their privacy policy.
